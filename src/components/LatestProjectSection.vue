<template>
  <section class="text-white mt-20" id="projects">
    <div class="px-4 xl:pl-16">
      <div class="mb-4 md:flex md:justify-between xl:pr-16">
        ...
        <div class="flex space-x-4 mb-4 mt-5 md:mt-0">
          ...
          }" class="hover:text-primary" v-for="category in ['all', 'Web Development', 'Mobile App']" :key="category"
        ...
        <div v-for="(project, index) in filteredProjects" :key="index">
          <div class="h-52 md:h-[24rem] rounded-t-xl relative group" :style="{
            ...
            <div
              ...
              <button class="h-14 w-14 border-2 relative rounded-full border-[#ADB7BE] hover:border-white group/link"
    ...
    <Teleport to="body">
      <div v-if="previewProject" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80"
        @click.self="closePreview">
        <div class="relative max-w-3xl w-full mx-4">
          <button class="absolute -top-10 right-0 text-white text-3xl hover:text-gray-300" @click="closePreview">
            &times;
          </button>
          <div class="overflow-hidden max-h-[70vh] flex items-center justify-center rounded-xl select-none" :class="isDragging
            ? 'cursor-grabbing'
            : zoomLevel > 1
              ? 'cursor-grab'
              : 'cursor-default'
            ...
            <img :src="previewProject.image" :alt="previewProject.title" :style="{
              transform: `scale(${zoomLevel}) translate(${translateX / zoomLevel}px, ${translateY / zoomLevel}px)`,
              transition: isDragging ? 'none' : 'transform 0.2s ease',
            }" class="rounded-xl object-contain max-h-[70vh] origin-center pointer-events-none" draggable="false" />
          </div>
          ...
          <div class="flex items-center justify-center gap-3 mt-3">
            ...
            <button @click="zoomReset" :disabled="zoomLevel === 1 && translateX === 0 && translateY === 0
              "
